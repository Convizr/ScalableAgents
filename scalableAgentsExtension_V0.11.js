...
export const OrderSelectionExtension = {
    ...
    render: ({ trace, element }) => {
        ...
        // ✅ Extract return product IDs (split like orders)
        const returnProductIds = payloadObj.returnProductIds
            ? payloadObj.returnProductIds.split('|').map(ids => ids.split(',').map(id => id.trim()))
            : [];
        ...
        console.log('Parsed Product IDs:', returnProductIds);

        // ✅ Ensure orders, products, quantities, and IDs match
        if (
            orderNumbers.length !== returnProductTitles.length ||
            orderNumbers.length !== orderedQuantities.length ||
            orderNumbers.length !== returnProductIds.length
        ) {
            console.error('Mismatch between order numbers, products, quantities, and product IDs!');
            return;
        }
        ...
        orderNumbers.forEach((orderNum, index) => {
            ...
            orderCard.dataset.productIds = JSON.stringify(returnProductIds[index] || []);
            ...
            orderCard.addEventListener('click', () => {
                if (selectedOrder === orderCard) {
                    ...
                    const payload = {
                        ...
                        selectedProductIds: returnProductIds[index].join(', ') // ✅ Added to output payload
                    };
