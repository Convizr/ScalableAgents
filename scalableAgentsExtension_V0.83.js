...
export const OrderSelectionExtension = {
    ...
    render: ({ trace, element }) => {
        ...
        // ✅ Ensure orders, products, and quantities match
        if (orderNumbers.length !== returnProductTitles.length || orderNumbers.length !== orderedQuantities.length) {
            console.error('Mismatch between order numbers, products, and quantities!');
            return;
        }
        ...
        orderNumbers.forEach((orderNum, index) => {
            ...
            orderCard.innerHTML = `
                <div class="order-number">Order ${orderNum.replace(/^#/, '')}</div>
            ...
            orderCard.addEventListener('click', () => {
                if (selectedOrder === orderCard) {
                    ...
                    window.voiceflow.chat.interact({
                        type: 'complete',
                        payload: { selectedOrderNumber: null, selectedProducts: [], selectedQuantities: [] },
                    });
                    ...
                    const payload = {
                        ...
                        selectedQuantities: orderedQuantities[index] || [],
                    };
