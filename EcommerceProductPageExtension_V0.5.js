export const ProductPageExtension = {
    ...
    render: ({ trace, element }) => {
      ...
      // Check if ShopifyProductData is still a placeholder string
      if (
        typeof payloadObj.ShopifyProductData === "string" &&
        payloadObj.ShopifyProductData.trim() === "{rawProductData}"
      ) {
        console.error("rawProductData variable was not expanded into the actual JSON data. Please check your Voiceflow configuration.");
        element.innerHTML = "<p>Error: rawProductData not provided.</p>";
        return;
      }
  
      // Ensure the payload's ShopifyProductData is an array
